"""
Test Case 20: Elderly Care Facility Order
Care facility manager buying comfortable clothing for elderly residents
"""

import pytest
from playwright.sync_api import Page
from Pages.All_pages import (
    LoginPage, RegistrationPage, ProductsPage,
    CartPage, CheckoutPage, PaymentPage, TestDataFactory
)


class TestElderlyCareOrder:
    """Test 20: Care facility manager buying comfortable clothing for elderly residents"""

    @pytest.fixture(autouse=True)
    def setup(self, page: Page):
        self.page = page
        self.login_page = LoginPage(page)
        self.registration_page = RegistrationPage(page)
        self.products_page = ProductsPage(page)
        self.cart_page = CartPage(page)
        self.checkout_page = CheckoutPage(page)
        self.payment_page = PaymentPage(page)

    def test_senior_resident_clothing(self):
        """Care facility ordering easy-wear clothing for senior residents"""
        user_data = TestDataFactory.get_user_data_canada()
        user_data['name'] = 'Margaret Thompson'
        user_data['address']['company'] = 'Sunrise Senior Living Center'
        user_data['address']['first_name'] = 'Margaret'
        user_data['address']['last_name'] = 'Thompson'

        self.login_page.navigate()
        self.login_page.signup(user_data['name'], user_data['email'])
        self.registration_page.fill_account_info('Mrs', user_data['password'], user_data['dob'])
        self.registration_page.fill_address_info(user_data['address'])
        self.registration_page.submit_registration()
        self.registration_page.continue_after_registration()

        # Order comfortable, easy-to-wear items for residents
        self.products_page.navigate()
        self.products_page.filter_by_category("Women", "Tops")
        self.products_page.add_nth_product(1, True)
        self.products_page.add_nth_product(2, True)

        self.products_page.navigate()
        self.products_page.filter_by_category("Men", "Tshirts")
        self.products_page.add_nth_product(1, True)
        self.products_page.add_nth_product(2, True)
        self.products_page.add_nth_product(3, False)

        self.products_page.go_to_cart()
        self.cart_page.proceed_to_checkout()
        self.checkout_page.add_comment(
            "SENIOR CARE FACILITY ORDER\n"
            "Facility: Sunrise Senior Living Center\n"
            "Department: Resident Services\n"
            "Manager: Margaret Thompson\n"
            "Purpose: Clothing for residents with limited mobility\n"
            "Requirements:\n"
            "- Easy to put on/remove (front opening preferred)\n"
            "- Soft, non-irritating fabrics\n"
            "- No small buttons (arthritis considerations)\n"
            "- Loose fit for comfort\n"
            "- Machine washable (industrial laundry)\n"
            "Sizes needed: Various (L-XXL)\n"
            "Invoice to: Facility procurement dept\n"
            "PO Number: SLC-2025-0012"
        )
        self.checkout_page.place_order()
        self.payment_page.fill_payment_details(TestDataFactory.get_payment_data())
        self.payment_page.confirm_payment()
        assert self.payment_page.verify_success()


# Pytest configuration
@pytest.fixture(scope="session")
def browser_context_args(browser_context_args):
    return {**browser_context_args, "viewport": {"width": 1920, "height": 1080}, "no_viewport": False}


if __name__ == "__main__":
    pytest.main([__file__, "-v", "-s", "--headed"])